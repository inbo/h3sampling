...
fn extract_signed_rings(geometry: &Geometry) -> extendr_api::Result<Vec<SignedRing>> {
    ...
    let mut process_poly = |poly: &Polygon| {
        rings.push(SignedRing {
            coords: densify_and_transform_ring(poly.exterior()),
            sign: 1.0,
        });
    ...
    match geometry {
        Geometry::Polygon(p) => process_poly(p),
...
pub fn grts_sobol_sample_from_wkb(
    ...
    // Un-sort points back to their exact sequence generation order
    ...
    let sidxs: Vec<i32> = targets.iter().map(|t| t.id as i32).collect();

    let df = data_frame!(lon = lons, lat = lats, sobol_index = sidxs);
